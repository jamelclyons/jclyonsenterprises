...
export default defineConfig(({ mode }) => {
  ...
  if (isDev) {
    ...
    const productServicesDir = packagesRoot ? path.resolve(packagesRoot, 'products-services') : null;

    packagesDir = packagesRoot ? [uiuxDir, communicationsDir, gatewayDir, gitportDir, typesDir, locationsDir, scheduleDir, productServicesDir].filter(Boolean) as string[] : [];
    ...
    const productsServices = productServicesDir ? path.resolve(productServicesDir, 'src/index.ts') : null;

    packages = [uiux, communications, gateway, gitport, types, locations, schedule, productsServices].filter(Boolean) as string[];
    ...
    localPackages = {
      ...
      ...(productsServices && { '@the7ofdiamonds/products-services': productsServices }),
    };
  }
